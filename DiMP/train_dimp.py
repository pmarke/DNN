...
def train_dimp():
    ...
    dataset = GOT10kDataset(root=VAL_PATH, template_size=256, num_frames=3)
    ...
    for epoch in range(10):
        ...
        for batch in tqdm(loader, desc=f"Epoch {epoch+1}"):
            ...
            template = torch.stack([sample[0] for sample in batch['template_images']]).to(device)  # [B, C, H, W]
            search   = torch.stack([sample[1] for sample in batch['search_images']]).to(device)    # [B, C, H, W]
            gt_box   = torch.stack([sample[1] for sample in batch['search_bboxes']]).to(device)    # [B, 4]
